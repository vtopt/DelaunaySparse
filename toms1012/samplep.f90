...
! user-specified interpolation points using DELAUNAYSPARSEP. If response
! values are provided, the interpolant f_{DT}(q) is also computed for all
! q \in Q.
!
! Usage: ./samplep $(filepath)
!
...
IF (IR > 0) THEN
   TICK = OMP_GET_WTIME()
   ! Call DELAUNAYSPARSEP with INTERP_IN and INTERP_OUT.
   CALL DELAUNAYSPARSEP(D, N, PTS, M, Q, SIMPS, WEIGHTS, IERR, &
     ! Optional argument list.
     & INTERP_IN=INTERP_IN, INTERP_OUT=INTERP_OUT,             &
     & EPS=SQRT(EPSILON(0.0_R8)), EXTRAP=0.1_R8, RNORM=RNORM,  &
     & IBUDGET = 50000, CHAIN=.FALSE., EXACT=.TRUE., PMODE=1)
   TICK = OMP_GET_WTIME() - TICK
! Otherwise, just compute the simplices and weights.
ELSE
   TICK = OMP_GET_WTIME()
   ! Call DELAUNAYSPARSEP without INTERP_IN and INTERP_OUT.
   CALL DELAUNAYSPARSEP(D, N, PTS, M, Q, SIMPS, WEIGHTS, IERR, &
